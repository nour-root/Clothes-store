let new_arrival = (p, item_wish) => {
  return `<div class="swiper-slide">
          <div class="border-[1px] border-border-alt rounded-2xl p-4 flex flex-col space-y-2">
            ...
            <img src="${
              p.image[0]
                ...
                <a href="/details.html?p=${
                  p.id
                }" aria-label="Quick View" class="hover:after:block hover:before:block hover:*:text-white bg-container hover:bg-first rounded-full pt-[11px] pb-2 px-3 relative after:absolute after:hidden before:hidden after:content-[attr(aria-label)] after:p-[.625rem] after:rounded-[.25rem] after:-left-[16px] after:bottom-[112%] after:bg-first after:text-white after:text-tiny-size after:whitespace-nowrap before:absolute before:content-[''] before:-top-[6px] before:border-[.5rem] before:border-transparent before:border-t-first"><i class="fi fi-rr-eye text-xl"></i></a>
                  <button type="button" id="btn-wish-new-${
                    p.id
                  }" onclick="add_to_wishList(${p.id})"  ${
    item_wish === p.id ? 'aria-label="Added"' : 'aria-label="Add To Wislist"'
  }  class="hover:after:block hover:before:block hover:*:text-white bg-container hover:bg-first rounded-full pt-[11px] pb-2 px-3 relative after:absolute after:hidden before:hidden after:content-[attr(aria-label)] after:w-[90px] after:p-[.625rem] after:rounded-[.25rem] after:-left-[50%] after:bottom-[112%] after:bg-first after:text-white after:text-tiny-size after:whitespace-nowrap before:absolute before:content-[''] before:-top-[6px] before:border-[.5rem] before:border-transparent before:border-t-first"><i class="fi fi-rs-heart text-xl"></i></button>
                </div>
            </div>
          </a>
